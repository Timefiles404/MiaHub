...
import dev.timefiles.miaeco.terrain.infinitetensor.FloatTensor;
...
public final class LocalTerrainProvider {
    ...
    /** 自启动以来累计计算的张量窗口数（进度条标定用；实例未建返回 0）。 */
    public static long windowCount() {
        LocalTerrainProvider p = INSTANCE;
        return p == null ? 0 : p.pipeline.getTotalComputedWindowCount();
    }
    ...
    private HeightmapData genHeightmap(CacheKey key, int i1, int j1, int i2, int j2) {
        int scale = dev.timefiles.miaeco.terrain.TerrainConfig.scale();
