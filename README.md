...
/miah enable <插件>
/miah disable <插件>
/miah heartbeat <hour|day|week|off>
```

`/miah pull` 会优先从 PlugSite catalog 拉取最新插件列表，使下载站网页上传的安装插件也能进入 MiaHub 的安装/更新流程；失败时回退到 GitHub catalog。拉取成功后会立即通报当前已安装插件里有哪些可更新项：
...
`/miah install` 和 `/miah update` 在实际安装/更新前会自动执行一次 `pull`，避免使用过期 catalog 把插件装回旧版本。`/miah heartbeat <hour|day|week|off>` 可以设置后台定时刷新 catalog，配置会保存到 `plugins/MiaHub/config.yml`。
