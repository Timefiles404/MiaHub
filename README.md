...
| `miaskillpool` | `MiaSkillpool` | `0.2.4` | `miaskillpool-v0.2.4` | MythicMobs 技能池插件，命令 `/mias` |
...
MiaSkillpool 运行时依赖 MythicMobs。构建时只使用 `D:\Projects\MiaPlugins\references\MythicMobsPremium-5.12.0-SNAPSHOT.jar` 作为 `compileOnly` 依赖，不会把 MythicMobs 打包进 MiaSkillpool jar。
