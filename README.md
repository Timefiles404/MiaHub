...
| --- | --- | --- | --- | --- |
| `miahub` | `MiaHub` | `0.2.6` | `miahub-v0.2.6` | Mia 系列插件管理器，命令 `/miah` |
...
MiaHub 只管理 `catalog.json` 中注册的 Mia 系列插件。`install` 补全会隐藏已安装插件和 MiaHub 自身；`update` 补全只显示已安装且版本落后于 catalog 的插件。`/miah list` 会用树形子行显示 catalog 中声明的前置插件，并检测它们是否已加载或已放入 `plugins/` 目录；安装或更新前置缺失的插件时会在下载前拦截。
